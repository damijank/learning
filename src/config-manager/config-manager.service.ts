...
import { MissingOverrideError, InvalidConfigurationError, MissingEnvFileError } from './errors'
...
export class ConfigManager extends AbstractConfigManager {
    ...
    protected loadAndValidateEnvFile() {
        ...
        try {
            ...
            if (error.message === NO_ENV_FILE_ERROR) {
                if (!this.options.allowMissingEnvFile) {
                    ...
                    dbg.cfg('> No env file found.  All env vars will come from external environment, if defined.')
                    loadedConfig = {}
                }
            }
        }
        ...
        if (missingKeyErrors.length > 0 || validationErrors) {
            ...
            if (this.options.onError === 'throw') {
                ...
                throw new InvalidConfigurationError(missingKeyErrors, validationErrorMessages)
            } else {
    ...
    private processOptions() {
        ...
        if (typeof this.environment === 'undefined' && this.options.hasOwnProperty('defaultEnvironment')) {
        ...
        if (this.options.onError && !['continue', 'throw', 'exit'].includes(this.options.onError)) {
    ...
    private cascadeRequiredVars(loadedConfig, requiredConfig) {
        ...
        Object.entries(requiredConfig).forEach(([key, value]) => {
            resolveMap[key] = {
                ...
                default: requiredConfig[key].default ? requiredConfig[key].default : '--',
                resolvedFrom: this.procEnv[key] ? 'env' : updatedConfig[key] ? 'dotenv' : '--',
                isExtra: false,
            }
        ...
        // eslint-disable-next-line no-unused-expressions
        this.traceProduction && dbg.cfg('> updatedConfig (after cascade): ', updatedConfig)
    ...
    private processConfigSpec(): Joi.SchemaMap {
        ...
        try {
            ...
            if (error instanceof MissingOverrideError) {
                errorMessage = 'Fatal error: required method provideConfigSpec missing from class extending ConfigService'
            } else {
        ...
        Object.keys(configSpec).map(key => {
            ...
            if (configSpec[key].required) {
                ...
                if (configSpec[key].default === 'undefined') {
        ...
        dbg.cfg(`> Loaded ${Object.keys(configSpec).length} configuration spec keys.`)
